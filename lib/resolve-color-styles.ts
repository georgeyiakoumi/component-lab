/**
 * Runtime colour resolution — converts Tailwind colour classes to inline styles
 * for the canvas preview. This avoids safelisting thousands of colour utilities.
 *
 * The function separates classes into:
 * - Remaining classes (non-colour, passed through as className)
 * - Inline styles (colour classes converted to CSS properties)
 */
...
/** Resolve a colour suffix (e.g. "blue-500") to hex */
function resolveColorHex(suffix: string): string | null {
  if (suffix === "black") return "#000000"
  if (suffix === "white") return "#ffffff"
  if (suffix === "transparent") return "transparent"
  if (suffix === "inherit") return "inherit"
  if (suffix === "current") return "currentColor"
...
const GRADIENT_PREFIX_TO_CSS: Record<string, string> = {
  "from": "--tw-gradient-from",
  "via": "--tw-gradient-via",
  "to": "--tw-gradient-to",
}

/** Gradient direction → CSS value */
...
/** Known non-colour suffixes for text- and bg- prefixes to avoid false matches */
...
function isNonColorSuffix(suffix: string): boolean {
  return NON_COLOR_SUFFIXES.has(suffix)
}
...
/**
 * Resolve colour classes to inline styles, returning remaining classes + a style object.
 * shadcn semantic tokens (e.g. text-foreground, bg-primary) are LEFT as classes
 * since they resolve via CSS custom properties and are already compiled by Tailwind.
 */
export function resolveColorStyles(
  classes: string[],
): { remainingClasses: string[]; style: React.CSSProperties } {
  ...
  for (const cls of classes) {
    ...
    if (cls.startsWith("opacity-")) {
      const val = cls.replace("opacity-", "")
      const num = parseInt(val, 10)
      if (!isNaN(num)) {
        style.opacity = String(num / 100)
        resolved = true
      }
    }

    // Gradient direction
    if (!resolved && GRADIENT_DIRECTION_MAP[cls]) {
      style.backgroundImage = GRADIENT_DIRECTION_MAP[cls]
      resolved = true
    }
    ...
    if (!resolved) {
      ...
      for (const prefix of prefixes) {
        ...
        // Didn't match — not a colour class, leave it
        break
      }
    }

    // Gradient colour stops (from-, via-, to-)
    if (!resolved) {
  ...
  // If we have gradient colours, build the stops CSS variable
  const from = style["--tw-gradient-from"]
  const to = style["--tw-gradient-to"]
  const via = style["--tw-gradient-via"]
  if (from || via || to) {
    const fromColor = from ?? "transparent"
    const toColor = to ?? "transparent"
    if (via) {
      style["--tw-gradient-stops"] = `${fromColor}, ${via}, ${toColor}`
    } else {
      style["--tw-gradient-stops"] = `${fromColor}, ${toColor}`
    }
  }

  return { remainingClasses: remaining, style: style as React.CSSProperties }
}
